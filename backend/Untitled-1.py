...
if __name__ == "__main__":
    ...
    print(f"📄 Found {len(resume_files)} anonymized resumes in {RESUMES_DIR}")
    print(f"📋 Found {len(jd_files)} JDs in {JDS_DIR}")

    if not resume_files:
        print(f"❌ No resume .txt files found in {RESUMES_DIR}")
        print("   Make sure you ran the anonymizer first.")
        exit(1)
    if not jd_files:
        print(f"❌ No JD .txt files found in {JDS_DIR}")
        print("   Make sure you ran the JD scraper first.")
        exit(1)

    pairs = pair_resumes_jds()
    build_excel(pairs, OUTPUT_PATH)
    print(f"\n✅ Open {OUTPUT_PATH} and fill the 'Fit Label' column.")